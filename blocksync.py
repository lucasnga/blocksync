...
if callable(getattr(os, "posix_fadvise", False)):
    ...
    def fadvise(fileobj, offset, length, advice):
        return posix_fadvise(fileobj.fileno(), offset, length, advice)
else:
    try:
        from fadvise import set_advice, POSIX_FADV_NOREUSE, POSIX_FADV_DONTNEED

        def fadvise(fileobj, offset, length, advice):
            return set_advice(fileobj, advice, offset, length)
    except:
        fadvise = None
...
def server(dev, deleteonexit, options):
    ...
    if options.hash.lower().startswith('xxh'):
        hash1 = getattr(xxhash, options.hash.lower())
    else:
